...
from welearn_datastack.data.enumerations import URLRetrievalType
...
def main() -> None:
    logger.info("DocumentCollectorHub generate batch URL starting...")
    ...
    mode_str: str = os.getenv("RETRIEVAL_MODE", "NEW_MODE")
    ...
    # Mode
    mode: URLRetrievalType = URLRetrievalType[mode_str.upper()]
    ...
    # Get URLs from DB
    logger.info("Retrieve URLs from DB")
    ids_to_batch = retrieve_data_from_database.retrieve_urls_ids(
        db_session, mode=mode, corpus_name=corpus_name, qty_max=qty_max
    )
    logger.info("'%s' URL were retrieved", len(ids_to_batch))
