...
def historicize_materialized_view(view_name: str):
    logger.info(f"Starting historicization of materialized view: {view_name}")
    ...
    try:
        # Assuming the materialized view has a timestamp column named 'created_at'
        statement = text(
            f"""
            INSERT INTO historical_{view_name} 
            SELECT * FROM {view_name} 
        """
        )
        db_session.execute(statement)
        db_session.commit()
        logger.info(f"Successfully historicized materialized view: {view_name}")
...
def main():
    ...
    historicize_materialized_view(view_name)
